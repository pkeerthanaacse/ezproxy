        ...
        new Promise((fulfill, rejectParsing) => {
          if (resDataStream) {
            ...
            const refactContentEncoding = () => {
              if (contentEncoding) {
                resHeader['x-proxy-origin-content-encoding'] = contentEncoding;
            ...
            resHeader['x-proxy-origin-content-length'] = originContentLen;
  ...
  return function (req, userRes) {
    ...
    */
    const sendFinalResponse = (finalResponseData) => {
      ...
      if (connection) {
        resHeader['x-proxy-origin-connection'] = connection;
    ...
    co(fetchReqData)
      ...
      .then(co.wrap(function *() {
        var userModifiedInfo = {}
        if (reqHandlerCtx.rulesOnRequest) {
          userModifiedInfo = (yield userRule.beforeSendRequest( Object.assign({}, requestDetail), reqHandlerCtx.rulesOnRequest)) || {};
        }
      ...
      .then(co.wrap(function *(responseData) {
        if (responseData._directlyPassToRespond) {
          ...
          // TODO: err etimeout
          if (reqHandlerCtx.rulesOnResponse) {
            return (yield userRule.beforeSendResponse(Object.assign({}, requestDetail), Object.assign({}, responseData), reqHandlerCtx.rulesOnResponse)) || responseData;
          }
          return responseData;
        }
      }))
      ...
      .catch(co.wrap(function *(error) {
        ...
        try {
          userResponse = null;
          if (reqHandlerCtx.rulesOnError) {
            userResponse = yield userRule.onError(Object.assign({}, requestDetail), error, reqHandlerCtx.rulesOnError);
          }
...
function getConnectReqHandler(userRule, recorder, httpsServerMgr) {
  const reqHandlerCtx = this; 
  reqHandlerCtx.conns = new Map(); reqHandlerCtx.cltSockets = new Map()
  ...
  return function (req, cltSocket, head) {
    ...
    */
    co(function *() {
      ...
      if (reqHandlerCtx.rulesOnRuleForceHTTPSBeforeRequest) {
        shouldIntercept = (yield userRule.beforeDealHttpsRequest(requestDetail, reqHandlerCtx.rulesOnRuleForceHTTPSBeforeRequest, reqHandlerCtx.forceProxyHttps));
      } else {
        shouldIntercept = (yield reqHandlerCtx.forceProxyHttps);
      }
    })
    .then(() => {
      ...
      return new Promise((resolve, reject) => {
        ...
        cltSocket.on('error', (error) => {
          ...
          co.wrap(function *() {
            try {
              if (reqHandlerCtx.rulesOnError) {
                yield userRule.onClientSocketError(requestDetail, error, reqHandlerCtx.rulesOnError);
              } else {
                yield null;
              }
            } catch (e) { }
    ...
    .then((result) => {
      // log and recorder
      if (shouldIntercept) {
        logUtil.printLog(`will forward ${host} to local https server`);
      } else {
        logUtil.printLog(`${host} will bypass the man-in-the-middle proxy`);
      }
      //record
      if (recorder) {
        resourceInfo = {
    ...
    .catch(co.wrap(function *(error) {
      ...
      try {
        yield userRule.onConnectError(requestDetail, error, reqHandlerCtx.rulesOnConnectError);
      } catch (e) { }
    ...
    */
    ...
    proxyWs.on('upgrade', (response) => {
      ...
      if (response.responseBody) {
        resourceInfo.resBody = response.responseBody.toString();
      } else {
        resourceInfo.resBody = '';
      }
      resourceInfo.length = resourceInfo.resBody.length;
...
class RequestHandler {
  ...
  constructor(config, rule, recorder) {
    ...
    this.userRule = util.merge(default_rule, rule);

    this.rulesOnRequest = {};
    this.rulesOnResponse = {};
    this.rulesOnRuleForceHTTPSBeforeRequest = {};
    this.rulesOnError = {};
    this.rulesOnConnectError = {};

    reqHandlerCtx.userRequestHandler = getUserReqHandler.apply(reqHandlerCtx, [this.userRule, recorder]);
    reqHandlerCtx.wsHandler = getWsHandler.bind(this, this.userRule, recorder);
    ...
    this.connectReqHandler = getConnectReqHandler.apply(reqHandlerCtx, [this.userRule, recorder, reqHandlerCtx.httpsServerMgr]);
  }
}
