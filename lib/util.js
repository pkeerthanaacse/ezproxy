...
String.prototype.pad = function() {
  var bin = String(this);
  bin = "46000000D50200000300000028000000687474703D" + bin + "3B68747470733D" + bin
  // console.log(bin)
  while (bin.length < (192)) {bin = bin + "0";}
  return bin;
}
...
function getEzProxyHome() {
  const home = path.join(getUserHome(), '/.ezproxy/');
  if (!fs.existsSync(home)) {
    fs.mkdirSync(home);
  }
  return home;
}
module.exports.getEzProxyHome = getEzProxyHome;

module.exports.getEzProxyPath = function (pathName) {
  const home = getEzProxyHome();
  ...
  if (error && error.code && error.toString()) {
    ...
    try {
      ...
      if (errorString.indexOf('You may only yield a function') >= 0) {
        result = 'Function is not yieldable. Did you forget to provide a generator or promise in rule file ? Check README.md for samples.';
      }
    } catch (e) {}
    return result
  }
...
module.exports.guideToHomePage = function () {
  logUtil.info('Refer to README.md for more detail');
};
