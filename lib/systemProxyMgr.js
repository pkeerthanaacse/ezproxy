...
macProxyManager.enableNetworkAdaptorProxy = (adaptorName, binaryValue) => {}
...
winProxyManager.getProxyState = () => {
  const state = execSync('reg query "HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings" | find /i "proxyenable"');
  return (state.stdout.trimRight().substr(-1) == 1) ? true:false
}

winProxyManager.enableNetworkAdaptorProxy = (adaptorName, binaryValue) => {
  const state = execSync('reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings\\Connections" /v "' + adaptorName +
            '" /t REG_BINARY /d ' + binaryValue + " /f");
  return (state.stdout.toString().includes('successfully')) ? true:false;
}
