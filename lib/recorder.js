...
function normalizeInfo(id, info) {
  ...
  singleRecord['rawRecord'] = `
      statusCode: ${singleRecord.statusCode}\n
      protocol: ${singleRecord.protocol}\n
      resHeader: ${singleRecord.resHeader}\n
      resBody: ${singleRecord.resBody}\n
      reqBody: ${singleRecord.reqBody}\n
      reqHeader: ${singleRecord.reqHeader}\n
      endTime: ${singleRecord.endTime}\n
      length: ${singleRecord.length}\n
      mime: ${singleRecord.mime}\n
...
class Recorder extends events.EventEmitter {
  constructor(config) {
    ...
    console.log("Records Location : " + this.cachePath + 'data')
  ...
  */
  ...
  appendRecord(info) {
    if (info.req.headers.ezproxy_web_req) { //TODO request from web interface
      return -1;
    }
    const self = this;
    const db = self.db;
