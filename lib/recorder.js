...
class Recorder extends events.EventEmitter {
  ...
  performTestsOnRecord(record) {
    ...
    self.testSuite = {"parentUUID": self.rootUUID, "uuid":testSuiteUUID,"title":"[" + new Date(record.startTime).toString().split(" ").slice(4,5).join(":") + "] CONNECT " + 
                  record.host,"fullFile":"adadada","file":"","beforeHooks":[],"afterHooks":[],"tests":[],"suites":[],"passes":[],"failures":[],"pending":[],"skipped":[],
            ...
            if (testedCount < testCount) {
              if (testEnabled) {
                try {
                  ...
                  var test = {"parentUUID": testSuiteUUID, "title":eachTestName, "fullTitle":"This test will test if ","timedOut":false,"duration":"","state":"passed","speed":"fast",
                            "pass":false,"fail":false,"pending":false,"context": data,"code":"","err":{},"uuid":testUUID,"parentUUID":testSuiteUUID,"isHook":false,"skipped":false}
